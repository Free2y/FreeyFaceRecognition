...
class FaceRecognition(Resource):
    ...
    def post(self):
        ...
        #     return fromatJsonDumps({'code': 999, 'msg': '已经超出免费使用次数'})
        # else:
        #     request_time[remote_ip_now] += 1
        ...
        if img_uri is not None or imgbase64 is not None:
            try:
                ...
                return fromatJsonDumps(
                    {'code': 200, 'msg': '成功',
                     'data': {'raw_out': results,
                              'speed_time': round(time.time() - start_time, 2)}})
            except Exception as ex:
                error_log = fromatJsonDumps({'code': 400, 'msg': '产生了一点错误，请检查日志', 'err': str(ex)})
                logger.error(error_log, exc_info=True)
                return error_log
        else:
            return fromatJsonDumps({'code': 400, 'msg': '没有传入参数'})
